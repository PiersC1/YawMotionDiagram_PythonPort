...
# import NN script
from pacejkaNN import load_model

model = load_model("./model/norm_params.json", "./model/R20_tire_model.pth")
    ...
    for x in range(len(slip_angle_range)):
        ...
        for y in range(len(front_left_steer_deltas)):
            ...
            while (Ay is None) or (abs(float(Ay) - float(Ay_guess)) > 1e-3):
                ...
                fx_lf_N, fy_lf_N = model.compute_forces(FZ_lf_N, alpha_lf)
                fx_rf_N, fy_rf_N = model.compute_forces(FZ_rf_N, alpha_rf)
                fx_lr_N, fy_lr_N = model.compute_forces(FZ_lr_N, alpha_lr)
                fx_rr_N, fy_rr_N = model.compute_forces(FZ_rr_N, alpha_rr)
                mz_lf_Nm = 0
                mz_rf_Nm = 0
                mz_lr_Nm = 0
                mz_rr_Nm = 0
